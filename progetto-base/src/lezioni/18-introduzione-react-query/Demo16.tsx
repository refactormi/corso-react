...
const api = {
  ...
  toggleTodo: async (id: number): Promise<Todo | undefined> => {
  ...
  deleteTodo: async (id: number): Promise<Todo | undefined> => {
  ...
  return (
    <div>
      <div style={{ 
        ...
        <h3 style={{ margin: 0 }}>📝 Todo List ({todos?.length || 0})</h3>
        <button
          onClick={() => refetch()}
          style={{
            padding: '6px 12px',
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', gap: 8 }}>
      ...
      <button
        ...
        style={{
          padding: '8px 16px',
          backgroundColor: addMutation.isPending ? '#6c757d' : '#007bff',
          color: 'white',
          border: 'none',
          borderRadius: 4,
      ...
      >
        {addMutation.isPending ? '⏳' : '➕'} Aggiungi
      </button>
    </form>
  );
}
...
function TodoItem({ todo }: TodoItemProps): JSX.Element {
  ...
  const toggleMutation = useMutation<Todo | undefined, Error, void, MutationContext>({
  ...
  const deleteMutation = useMutation<Todo | undefined, Error, void, MutationContext>({
...
export default function Demo16(): JSX.Element {
  return (
