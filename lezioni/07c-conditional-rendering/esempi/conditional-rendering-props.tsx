...
function WelcomeMessage({ isLoggedIn, username = "Ospite" }: WelcomeMessageProps) {
  return (
    <div style={{ 
      padding: '15px', 
      borderRadius: '8px',
...
function NotificationBanner({ hasNotifications, notificationCount = 0 }: NotificationBannerProps) {
  return (
    <div>
...
function FilteredList({ items, filter = 'all' }: FilteredListProps) {
...
function Dashboard({ user, hasData, isLoading, error }: DashboardProps) {
  // Caso 1: Errore
  if (error) {
    return (
      <div style={{ 
